<?php
    ...
    if (!empty($_POST['Milestone']) && empty($_POST['FromDate']) && empty($_POST['ToDate'])) {
        ...
        $sql = "SELECT orderdetails.VegetableID, SUM(orderdetails.Quantity) AS `Quantity`, STR_TO_DATE(`OrderDate`,'%Y-%m-%d') AS `OrderDate`
                FROM ((`orderdetails`
                ...
                GROUP BY orderdetails.VegetableID, `ShipDate`
                ORDER BY `OrderDate`, SUM(orderdetails.Quantity) DESC";
        $result = $classOrder->executeResult($sql); 
        if (is_array($result) || is_object($result)) {
            foreach ($result as $statistic) {
                $chart_data[] = array (
                    'VegetableID' => $statistic['VegetableID'],
