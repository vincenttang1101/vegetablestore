<?php 
    ...
    if (isset($_POST['VegetableID'])) {
        ...
        $HiddenVegetable = $classVegetable->getByID($VegetableID);
        foreach ($HiddenVegetable as $Vegetable) {
            if ($Vegetable['Hidden'] == "no") {
                $result_hidden = $classVegetable->HiddenVegetable($VegetableID,"yes");
            }
            else {
                $result_hidden = $classVegetable->HiddenVegetable($VegetableID,"no");
            }
        }
        $output_vegetablelist = '';
        if ($result_hidden) {
            $message = "The Vegetable has been successfully updated hidden";
            ...
            $output_vegetablelist .= "<div class='alert alert-danger' id='alert-danger'>The Vegetable has been unsuccessfully hiddened</div>";
        }
