<?php 
    ...
    if (!empty($_POST)) {
        ...
        $Unit               = $_POST['Unit'];
        $Amount             = $_POST['Amount'];
        $Image              = 'images/' . basename($_FILES['UploadFile']['name']);
        $Price              = $_POST['Price'];
        $output_vegetablelist = '';
        $DataVegetable = array($CategoryID, $VegetableName, $Unit, $Amount, $Image, $Price);
        $result_addvegetable = $classVegetable->addVegetable($DataVegetable);
        if ($result_addvegetable) {
            ...
            $output_vegetablelist .= "<div class='alert alert-danger' id='alert-danger'>The Vegetable has been unsuccessfully added</div>";
        }
